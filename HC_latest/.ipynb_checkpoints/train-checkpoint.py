...
#全局特征
        ...
        optimizer.zero_grad()
        torch.autograd.backward([loss0, loss1, loss2, loss3, loss4,loss_id], [torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda(), torch.tensor(1.0).cuda()])
        ...
        if batch_idx%10 ==0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  data_time=data_time, train_loss=train_loss, tri_loss=tri_loss, id_loss=id_loss,part_id_loss=part_id_loss))
